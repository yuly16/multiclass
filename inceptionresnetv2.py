...
class Multinet(nn.Module):
    def __init__(self, num_classes=20):
        ...
        self.old_module = InceptionResNetV2()
        self.last_linear1 = nn.Linear(1000,20)
    def forward(self,x):
        x = self.old_module(x)
...
def inceptionresnetv2(num_classes=1000, pretrained='imagenet'):
    ...
    if pretrained:
        ...
        model.old_module.load_state_dict(torch.load("inceptionresnetv2-520b38e4.pth"))


        if pretrained == 'imagenet':
            new_last_linear = nn.Linear(1536,1000)
            model.old_module.last_linear = new_last_linear

        model.old_module.input_space = settings['input_space']
        model.old_module.input_size = settings['input_size']
        model.old_module.input_range = settings['input_range']
